...
test("toFile svg", function (t) {
    ...
    var expectedOutput = fs.readFileSync(path.join(__dirname, "/svg.expected.out"), "UTF-8");
...
test("toFile ttf", function (t) {
    var fileName = path.join(tmpDir(), "ttfimage.txt");
    var expectedOutput = [
        "_X[[[X_ ^SU^QSXR] _X[[[X_",
        "^RZZZR^ ZTRTSUZ _ ^RZZZR^",
        "[U YZZZ]Y[\\WRV]WZ\\RW\\W]][",
        "_VQU]UZ^WVRSZRV_\\ZXQWRSV[",
        "[RZZZZ[X_^_VUTYQ_XZX_RX]S",
        "_ ^^^ _ \\S^ ZX_\\Z^]_[\\S__",
